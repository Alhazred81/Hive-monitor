...
// ─── HTML Fejléc és Navigációs menü (4 füllel) ───────────────────
String htmlHead(const String& title, const String& activeTab) {
  ...
  // A 4 kiválasztott fül
  s += "<a href='/'" + String(activeTab=="1"?" class='on'":"") + ">🏠 Főoldal</a>";
  s += "<a href='/config'" + String(activeTab=="2"?" class='on'":"") + ">📡 Kapcsolatok</a>";
  s += "<a href='/sensors'" + String(activeTab=="3"?" class='on'":"") + ">🌡 Szenzorok</a>";
  s += "<a href='/diag'" + String(activeTab=="4"?" class='on'":"") + ">🩺 Diag</a>";
...
// ─── CSS Stíluslap kiszolgálása (Aszinkron módon) ──────────────
